package maen.edu.rental_house.ui_Rental.home;
...
public class HomeFragment extends Fragment implements SetOnRecyclerViewClickListener {
    ...
    public void setOnRecyclerViewListClicked(View view, int position) {
        ...
        if (myHouse.getItemCount() != 0 ){
            bundle.putSerializable("House", myHouse.getItem(position));
            Navigation.findNavController(view).navigate(R.id.nav_getHouse,bundle);
        } else {
            bundle.putSerializable("House_Info", myPost.getItem(position));
