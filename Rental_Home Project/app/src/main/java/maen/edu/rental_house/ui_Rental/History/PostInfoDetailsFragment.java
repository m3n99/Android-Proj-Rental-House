...
public class PostInfoDetailsFragment extends Fragment {
    ...
    public PostInfoDetailsFragment() {
    ...
    public View onCreateView(LayoutInflater inflater, ViewGroup container,
        ...
        View view = inflater.inflate(R.layout.fragment_postinfo_details, container, false);
        ...
        Button buttonPostReserve = view.findViewById(R.id.buttonPostReserve);
        ImageView Image = view.findViewById(R.id.PhotoUpload);
        assert bundle != null;
        Post_Info post_inf = (Post_Info) bundle.getSerializable("House_Info");
        ...
        emailAgency = post_inf.getEmailAgency();
        System.out.println(emailAgency);

        String NameAgency = dataBaseHelper.getAgency(emailAgency).getAName();
        System.out.println(NameAgency);

        emailTenant=SignIn.EmailAddressTenant;
        FirstName= dataBaseHelper.getTenant(emailTenant).getFName();
        LastName = dataBaseHelper.getTenant(emailTenant).getLName();

        buttonPostReserve.setOnClickListener(new View.OnClickListener() {
            public void onClick(View view) {
                Reserve reserve = new Reserve();
                if(!dataBaseHelper.isReserved(textViewPostalCity.getText().toString(),NameAgency)){
                    reserve.setCity(textViewPostalCity.getText().toString());
                    reserve.setPostal(post_inf.getPostal());
                    String date = getCurrentDateTime();
                    reserve.setDate(date);
                    reserve.setAgencyName(NameAgency);
                    reserve.setFirstName(FirstName);
                    reserve.setLastName(LastName);
                    reserve.setEmailAgency(emailAgency);
                    reserve.setEmailTenant(emailTenant);
                    dataBaseHelper.insertReserve(reserve);
                    post_inf.setReserved(true);
